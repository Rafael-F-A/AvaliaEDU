// PONTE DE CONEXÃO: Traduz as chamadas do Dashboard para o sistema da API
async function apiFetch(endpoint, opcoes = {}) {
  const API_BASE_URL = 'http://localhost:8000'; // Endereço do seu Python
  ...
  if (opcoes.body) {
    corpo = typeof opcoes.body === 'string' ? opcoes.body : JSON.stringify(opcoes.body);
  }

  const response = await fetch(url, { method: metodo, headers, body: corpo });
...
/* GEOLOCALIZAÇÃO E LISTA DE LOCAIS  */
